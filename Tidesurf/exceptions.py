...
class ExchangeError(DependencyException):
    """
    Error raised out of the exchange_adapter.
...
class InvalidOrderException(ExchangeError):
    ...
    If stoploss on exchange_adapter order is hit, then trying to cancel the order
...
class InsufficientFundsError(InvalidOrderException):
    """
    This error is used when there are not enough funds available on the exchange_adapter
    to create an order.
    """
...
class TemporaryError(ExchangeError):
    """
    Temporary network or exchange_adapter related error.
    This could happen when an exchange_adapter is congested, unavailable, or the user
